            ...
            <div className="max-w-7xl mx-auto px-6 lg:px-8">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
                    {industries.map((industry, idx) => (
                        <div
                            key={idx}
                            className={`flex items-start gap-4 p-6 rounded-2xl bg-white/[0.02] border border-white/5 transition-all duration-500 card-3d glow-border-hover stagger-${idx + 1} ${isVisible ? 'reveal-visible' : 'reveal-hidden'}`}
                        >
                            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-cyan-500/20 to-blue-600/20 flex items-center justify-center text-cyan-400 flex-shrink-0 icon-animated">
                                {industry.icon}
                            </div>
                            <div>
